...
module.exports = function(Allchart) {

    Allchart.createChart = (params, cb) => {
        ...
        if (params) {
            ...
            let { happeningsId, customerId, type , category , sportsDateId, timeZone = "Australia/Sydney" ,  
            ...
            let momentdate = new Date(),
               ...
               let  createChartAll = ({ age , gender ,  customerId , aCCRes , ownerId }) =>{
                    ...
                    if (aCCRes && aCCRes.id) {
                        ...
                        if (type == 'view') {
                            alchaobj.view = Number(alchaobj.view || 0) + 1;
                        } else if (type == 'interest') {
                            alchaobj.interest = Number(alchaobj.interest || 0) + 1;
                        }

                        updateObj[`_${hour}to${_hoursPlus}`] = alchaobj;
                        ...
                        //UpdateData 
                          Allchart.upsertWithWhere({ id: aCCRes.id }, updateObj);

                         AllCategoryCustomerHits.create({
                            allChartId: aCCRes.id, customerId, scanDateZero: dateZero, date, month, year,
                            scanDate: dateStr, scanTime: timeStr, country, timeZone
                        }, ()=>{
                                if (ownerId && date && month && year) {

                                    let alCFilter = {};
                                    if(category == "Happenings") {
                                        alCFilter = { ownerId, happeningsId , date, month, year };
                                    } else if(category == 'Sports'){
                                        alCFilter = { ownerId, sportsDateId , date, month, year };
                                    } 

                                    Allchart.findOne({ where : alCFilter , 
                                        ...
                                        if (AllCChartD && AllCChartD.allCategoryCustomerHits && AllCChartD.id) {
                                            ...
                                            if (newCnt && newCnt.length > 1) {
                                                ...
                                                Allchart.upsertWithWhere({ id: AllCChartD.id }, { returning })
                                            } else {
                                                let newCount = Number(AllCChartD.new || 0) + 1;
                                                Allchart.upsertWithWhere({ id: AllCChartD.id }, { new: newCount })
                                            }
                                        }
                                    })
            ...
            if(customerId){
                Customer.findOne({ where: { id: customerId } }, { fields: ["id", "firstName", "lastName", "mobile", "postCode", "dob", "age", "state", "city", "gender"] }, (Cuerr, Cures) => { 
                    if(Cuerr) { isCallBack(false, "Enter valid customer id"); }
                        ...
                        if (happeningsId && category == 'Happenings') {
                            Happenings.find({ where: { id: happeningsId } }, (er, rs) => {
                                if (er) isCallBack(false, "Error", er);
                                    ...
                                    if (rs && rs.length) {
                                        ...
                                        Allchart.upsertWithWhere({ ownerId, date, month, year, happeningsId , type : category }, { customerId, ownerId, happeningsCategoryId, happeningsId, dateStr, timeStr, timeZone, date, month, year, dateZero, countryDate, hour, minute, day , country , type : category }, (aCCErr, aCCRes) => {
                                            if (aCCRes) {
                                                createChartAll({ age: Cures.age, gender: Cures.gender, customerId, aCCRes, ownerId });
                                            } 
                                        });
                        ...
                        else if(sportsDateId && category == 'Sports'){
                             ...
                             SportsDate.findOne({ where : { id : sportsDateId  } , fields : ["id", "sportsId"] , include : [{ relation : "sports" , scope : { fields : ["sportsCategoryId", "ownerId",  "id"] } }] }, (sEr , sRe)=>{
                                ...
                                if(sRe && sRe.id && sRe.sportsId) {
                                    ...
                                    Allchart.upsertWithWhere({ ownerId, date, month, year , sportsDateId , type : category }, { customerId, ownerId, sportsCategoryId, sportsId , sportsDateId , dateStr, timeStr, timeZone, date, month, year, dateZero, countryDate, hour, minute, day , country , type : category }, (aCCErr, aCCRes) => {
                                        if (aCCRes) {
                                            createChartAll({ age: Cures.age, gender: Cures.gender, customerId, aCCRes, ownerId });
                                        }
                                    });
    ...
    Allchart.remoteMethod('createChart', {
        http: { path: '/createChart', verb: 'post' },
