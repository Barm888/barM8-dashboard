angular
    .module('app')
    .controller('analyticsCtl', ['$scope', '$state', '$rootScope', 'Business', '$http', 'VisitCount', 'socket', 'BusinessVisitCount',
        function ($scope, $state, $rootScope, Business, $http, VisitCount, socket, BusinessVisitCount) {
            ...
            $scope.dailyChart = (arg = {}) => {
                let { _00to01 = 0, _01to02 = 0, _02to03 = 0, _03to04 = 0, _04to05 = 0, _05to06 = 0, _06to07 = 0, _07to08 = 0, _08to09 = 0, _09to010 = 0, _10to11 = 0, _11to12 = 0,
                    _12to13 = 0, _13to14 = 0, _14to15 = 0, _15to16 = 0, _16to17 = 0, _17to18 = 0, _18to19 = 0, _19to20 = 0, _20to21 = 0, _21to22 = 0, _22to23 = 0, _23to24 = 0 } = arg;


                let greaterObj = {
                    _00to01, _01to02, _02to03, _03to04, _04to05, _05to06, _06to07, _07to08, _08to09, _09to010, _10to11, _11to12,
                    _12to13, _13to14, _14to15, _15to16, _16to17, _17to18, _18to19, _19to20, _20to21, _21to22, _22to23, _23to24
                };

                let key = Object.keys(greaterObj).reduce(function (a, b) { return greaterObj[a] > greaterObj[b] ? a : b }), maxLength = 0;
                if (key) maxLength = parseFloat(greaterObj[key]);

                let maxLengthString = maxLength.toString();

                roundNumber = (arg) => {
                    if (arg.startsWith("8")) {
                        if (arg.length == 3) arg = "900";
                        else if (arg.length == 4) arg = "9000";
                        return arg;
                    }
                    else if (arg.startsWith("7")) {
                        if (arg.length == 3) arg = "800";
                        else if (arg.length == 4) arg = "8000";
                        return arg;
                    }
                    else if (arg.startsWith("6")) {
                        if (arg.length == 3) arg = "700";
                        else if (arg.length == 4) arg = "7000";
                        return arg;
                    }
                    else if (arg.startsWith("5") || arg.startsWith("4") || arg.startsWith("3") || arg.startsWith("2") || arg.startsWith("1")) {
                        if (arg.length == 3) arg = "600";
                        else if (arg.length == 4) arg = "6000";
                        return arg;
                    }
                    else return arg;
                }

                maxLength = parseInt(roundNumber(maxLengthString));
                ...
                //Age Chart
                let { _18to23_Age = 0, _24to29_Age = 0, _30to35_Age = 0, _36to41_Age = 0, _42toPlus_Age = 0 } = arg;
                var pieAgeDaily = {
                    "series": ["Visit"],
                ...
                // Male and female
                let { male = 0, female = 0, others = 0 } = arg;
            ...
            getVisitCount = () => {
                ...
                VisitCount.find({ filter: { where: { ownerId: $scope.userId, date: ("0" + newdate.getDate()).slice(-2), month: ("0" + (newdate.getMonth() + 1)).slice(-2), year: newdate.getFullYear() } } }).$promise.then((res) => {
                    if (res && res.length) {
                        $scope.dailyChart(res[0]);
                    } else $scope.dailyChart();
                })
            }

            //barLiveChart
            socket.on('pushanalytics', function (data) {
                getVisitCount();
            })

            var weekStart = new Date(), weekend = new Date();

            weekCharts = (arg = []) => {
                let _week_day_0 = 0, _week_day_1 = 0, _week_day_2 = 0, _week_day_3 = 0, _week_day_4 = 0, _week_day_5 = 0, _week_day_6 = 0;
                ...
                arg.forEach(val => {
                    if (val.dateZero) {
                        ...
                        let totalCnt = _00to01 + _01to02 + _02to03 + _03to04 + _04to05 + _05to06 + _06to07 + _07to08 + _08to09 + _09to010 + _10to11 + _11to12
                            + _12to13 + _13to14 + _14to15 + _15to16 + _16to17 + _17to18 + _18to19 + _19to20 + _20to21 + _21to22 + _22to23 + _23to24;
                ...
                var barWeekLive = {
                    ...
                    options: {
                        ...
                        title: {
                            display: true,
                            text: 'Customer visit per Week'
                        }
                    }
                };
                $scope.barWeekly = barWeekLive;
            ...
            $scope.weekCharts = (firstWeek, lastWeek) => {
                weekStart = new Date(firstWeek);
                weekend = new Date(lastWeek);
            };

            weeklyChartVisitCount = () => {
                ...
                VisitCount.find({ filter: { where: { ownerId: $scope.userId, dateZero: { between: [dateZeroStart, dateZeroEnd] } } } })
                    .$promise.then((res) => {
                        if (res && res.length) {
                            weekCharts(res);
                        } else weekCharts();
            ...
            var monthlyCurrentMonth, monthlyCurrentYear, monthlyStart, monthlyEnd, monthlyMonthIndex;
            ...
            monthCharts = (arg = []) => {
                ...
                for (let i = monthlyStart; i <= monthlyEnd; i++) {
                ...
                $scope.barMonthly = barMonthlyLive;
            ...
            monthlyCharts = () => {
                VisitCount.find({ filter: { where: { ownerId: $scope.userId, year: { eq: monthlyCurrentYear }, month: { eq: monthlyMonthIndex }, date: { between: [monthlyStart, monthlyEnd] } } } })
                    .$promise.then((res) => {
                        if (res && res.length) {
                            monthCharts(res);
                        } else monthCharts();
            ...
            $scope.MonthlyCharts = (month, year, start, end, index) => {
                monthlyCurrentMonth = month;
                monthlyCurrentYear = parseInt(year);
                monthlyStart = parseInt(start);
                monthlyEnd = parseInt(end);
                monthlyMonthIndex = parseInt(index);
                monthCharts();
            }
            ...
            yearlyChart = (arg = []) => {
                ...
                let _Jan = _Feb = _Mar = _Apr = _May = _Jun = _Jul = _Aug = _Sep = _Oct = _Nov = _Dec =
                ...
                for (let i = 1; i <= 12; i++) {
                    ...
                    data.forEach(val => {
                        if (val) {
                            let { _00to01, _01to02, _02to03, _03to04, _04to05, _05to06, _06to07, _07to08, _08to09, _09to010, _10to11, _11to12
                                , _12to13, _13to14, _14to15, _15to16, _16to17, _17to18, _18to19, _19to20, _20to21, _21to22, _22to23, _23to24 } = val;
                            let totalCnt = _00to01 + _01to02 + _03to04 + _04to05 + _05to06 + _06to07 + _07to08 + _08to09 + _09to010 + _10to11 + _11to12
                                + _12to13 + _13to14 + _14to15 + _15to16 + _16to17 + _17to18 + _18to19 + _19to20 + _20to21 + _21to22 + _22to23 + _23to24;
                            if (i == 1) _Jan += totalCnt; if (i == 2) _Feb += totalCnt; if (i == 3) _Mar += totalCnt;
                            if (i == 4) _Apr += totalCnt; if (i == 5) _May += totalCnt; if (i == 6) _Jun += totalCnt;
                            if (i == 7) _Jul += totalCnt; if (i == 8) _Aug += totalCnt; if (i == 9) _Sep += totalCnt;
                            if (i == 10) _Oct += totalCnt; if (i == 11) _Nov += totalCnt; if (i == 12) _Dec += totalCnt;
            ...
            yearlyCharts = () => {
                VisitCount.find({ filter: { where: { ownerId: $scope.userId, year: yearlyCurrentYear } } })
                    .$promise.then((res) => {
                        if (res && res.length) {
                            yearlyChart(res);
                        } else yearlyChart();
            ...
            $scope.YearlyCharts = year => { yearlyCurrentYear = year; yearlyCharts(); }

            getBusinessDailyHits = (res = {}) => {
                ...
                if (res) {
                    ...
                    _00to01 = res._00to01; _01to02 = res._01to02; _02to03 = res._02to03; _04to05 = res._04to05; _05to06 = res._05to06;
                    _06to07 = res._06to07; _08to09 = res._08to09; _09to010 = res._09to010; _10to11 = res._10to11; _11to12 = res._11to12;
                    _12to13 = res._12to13; _13to14 = res._13to14; _14to15 = res._14to15; _15to16 = res._15to16; _16to17 = res._16to17;
                    _17to18 = res._17to18; _18to19 = res._18to19; _19to20 = res._19to20; _20to21 = res._20to21; _21to22 = res._21to22;
                    _22to23 = res._22to23; _23to24 = res._23to24;
                }
            ...
            getBusinessHitsFromModel = () => {
                //daily chart
                let newdate = new Date();
                BusinessVisitCount.findOne({ filter: { where: { ownerId: $scope.userId, date: ("0" + newdate.getDate()).slice(-2), month: ("0" + (newdate.getMonth() + 1)).slice(-2), year: newdate.getFullYear() } } })
                    .$promise.then((res) => {
                        getBusinessDailyHits(res);
                    });
                ...
                BusinessVisitCount.find({ filter: { where: { ownerId: $scope.userId, year: { eq: monthlyCurrentYear }, month: { eq: monthlyMonthIndex }, date: { between: [monthlyStart, monthlyEnd] } } } })
                    .$promise.then((res) => {
                        if (res && res.length) {
