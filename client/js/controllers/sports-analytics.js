angular
    .module('app')
    .controller('sportsAnalyticsCtl', ['$scope', '$state', '$rootScope', 'Business', '$http', 'Sports', 'socket', 'loader', '$filter', 'SportsCategory', 'AllChart', 'SportsDate',
        function ($scope, $state, $rootScope, Business, $http, Sports, socket, loader, $filter, SportsCategory, AllChart, SportsDate) {
            ...
            $scope.setChartdata = (arg = [], sDateArg, eDateArg) => {
                ...
                let _00to01_v = 0, _01to02_v = 0, _02to03_v = 0, _03to04_v = 0, _04to05_v = 0, _05to06_v = 0, _06to07_v = 0, _07to08_v = 0, _08to09_v = 0, _09to10_v = 0, _10to11_v = 0, _11to12_v = 0, _12to13_v = 0, _13to14_v = 0, _14to15_v = 0, _15to16_v = 0, _16to17_v = 0, _17to18_v = 0, _18to19_v = 0, _19to20_v = 0, _20to21_v = 0, _21to22_v = 0, _22to23_v = 0, _23to24_v = 0;

                let _18to23_Age = 0, _24to29_Age = 0, _30to35_Age = 0, _36to41_Age = 0, _42toPlus_Age = 0, newCnt = 0, returning = 0, male = 0, female = 0, others = 0;

                let _00to01_i = 0, _01to02_i = 0, _02to03_i = 0, _03to04_i = 0, _04to05_i = 0, _05to06_i = 0, _06to07_i = 0, _07to08_i = 0, _08to09_i = 0, _09to10_i = 0, _10to11_i = 0, _11to12_i = 0, _12to13_i = 0, _13to14_i = 0, _14to15_i = 0, _15to16_i = 0, _16to17_i = 0, _17to18_i = 0, _18to19_i = 0, _19to20_i = 0, _20to21_i = 0, _21to22_i = 0, _22to23_i = 0, _23to24_i = 0;
                ...
                if (arg && arg.length == 1) {
                    ...
                    arg.forEach((v, i) => {
                        ...
                        _18to23_Age = v._18to23_Age, _24to29_Age = v._24to29_Age, _30to35_Age = v._30to35_Age, 
                        _36to41_Age = v._36to41_Age, _42toPlus_Age = v._42toPlus_Age;

                        male = v.male, female = v.female, others = v.others;
                ...
                $('#s_bar_chart').remove();
                $('#bar-chart').append('<canvas id="s_bar_chart"><canvas>');

                new Chart(document.getElementById("s_bar_chart"), {
                    type: 'bar',
                    data: {
                        labels: labelsFor,
                        datasets: [{
            ...
            $scope.dateFromToChange = () => {
                if ($("#liveDateTo").val() && $("#liveDateFrom").val()) {
                    ...
                    let id = $("#sports_date_on_c").val();
                    if (id) {
                        AllChart.find({ filter: { where: { sportsDateId: id, dateZero: { between: [sdate, edate] } } } }).$promise.then((res) => {
                            if (res && res.length) {
            ...
            getDailyCount = (data, start, end) => {
                ...
                if (data && data.id) {
                    ...
                    AllChart.find({ filter: { where: { ownerId: $scope.userId, sportsDateId: data.id, dateZero: { between: [startDate, endDate] } } } }).$promise.then((res) => {
                        if (res && res.length) {
                            $scope.chartShow = true;
            ...
            $scope.dateOnChange = (data) => {
                if (data.id && data.date) {
                    let dateZero;
                    if (data.date) dateZero = `${data.date}T00:00:00.000Z`;
                    let startDate = `${data.date}T00:00:00.000Z`;
                    let endDate = `${data.date}T00:00:00.000Z`;
                    loader.visible();
                    AllChart.find({ filter: { where: { ownerId: $scope.userId, sportsDateId: data.id, dateZero } } }).$promise.then((res) => {
                        if (res && res.length) {
                            $scope.chartShow = true;
                            setTimeout(function () { loader.hidden(); }, 300);
                            $scope.setChartdata(res, startDate, endDate);
                        } else {
                            $scope.chartShow = false;
                            toastMsg(false, "Please try again!");
                            setTimeout(() => {
            ...
            $scope.getCategories = () => {
                ...
                SportsCategory.find().$promise.then((res) => {
                    $scope.categoryDropList = res.filter(m => m.group == 'Sports');
                    setTimeout(function () { loader.hidden(); }, 300);
                });
            }
            ...
            $scope.chartShow = false;
            $(".wrapper").css({ height: '50rem' });


            $scope.dateDisabled = true;
            $scope.sportsDksDates = [];
            $scope.sportsOnChange = (id) => {
                if (id) {

                    Sports.findOne({ filter: { where: { id, ownerId: $scope.userId }, include: [{ relation: "sportsDates" }] } }).$promise.then((res) => {

                        $scope.sportsDksDates = [];
                        for (let da of res.sportsDates) {
                            $scope.sportsDksDates.push({
                                value: `${da.dateFormat.split('T')[0]}`,
                                txt: `${$scope.formatDate(da.dateFormat)}`, id: da.id
                            });
                        }
                    });
                }
            ...
            $scope.lFromTodate = [];
            $scope.sportsSDateOnChange = (id) => {

                if (id) {
                    $scope.lFromTodate = [];

                    SportsDate.find({ filter: { where: { id } } }).$promise.then((res) => {
                        ...
                        if (res && res.length) {
                            ...
                            $scope.maxDate = (res[0].dateFormat).split('T')[0];

                            for (var day = new Date(res[0].liveDate.split('T')[0]); day <= new Date(res[0].dateFormat.split('T')[0]); day.setDate(day.getDate() + 1)) {
                                ...
                                if (cfDate == ftdate) {
                                    loader.visible();
                                    getDailyCount(res[0], ftdate.split('T')[0], ftdate.split('T')[0]);
                                } else {
                                    $scope.lFromTodate.push({
                                        sportsDateId: res[0].id,
            ...
            $scope.categoryOnChange = (id) => {
                if (id) {
                    $scope.sportsDataList = [];
                    loader.visible();
                    Sports.find({ filter: { where: { sportsCategoryId: id, ownerId: $scope.userId }, include: [{ relation: "sportsCategory" }, { relation: "sportsDates", scope: { where: { status: "Live" } } }], fields: ["id", "title", "sportsCategory", "sportsDates", "ownerId"] } }).$promise.then((res) => {
                        if (res) {
                            $scope.sportsDataList = res;
                            loader.hidden();
                        }
                    })
            ...
            if ($scope.userDetails.isAdmin == false) {
                ...
                if ($rootScope.selectedVenue && $rootScope.selectedVenue.venueId) {
                    ...
                    if ($scope.userId) $scope.getCategories();
                }
            }
