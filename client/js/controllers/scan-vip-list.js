angular
  .module('app')
  .controller('ScanVIPListCtl', ['$scope', 'Business', '$http', 'AuthService', '$state', '$rootScope', 'UserLoginDetails','LoyaltyLines', function ($scope, Business, $http, AuthService, $state, $rootScope, UserLoginDetails, LoyaltyLines) {
    if (!$scope.userId) {
    ...
    $scope.loyaltyDetails = [];
    $scope.bronzeCnt = 0; $scope.silverCnt = 0; $scope.goldCnt = 0; $scope.platinumCnt = 0; $scope.vipCnt = 0;
    $scope.getloyalty = () => {
      if ($scope.ownerId) {
        LoyaltyLines.find({ filter: { where: { ownerId: $scope.ownerId }, include: { relation: 'customer' } } }).$promise.then((res) => {
          if (res && res.length > 0) {
            $scope.loyaltyDetails = res.filter(m => m.customer);
            $scope.bronzeCnt = ($scope.loyaltyDetails.filter(m => m.level == 'Bronze')).length;
            $scope.silverCnt = ($scope.loyaltyDetails.filter(m => m.level == 'Silver')).length;
            $scope.goldCnt = ($scope.loyaltyDetails.filter(m => m.level == 'Gold')).length;
            $scope.platinumCnt = ($scope.loyaltyDetails.filter(m => m.level == 'Platinum')).length;
            $scope.vipCnt = ($scope.loyaltyDetails.filter(m => m.level == 'VIP')).length;
          }
        });
      }
    };
